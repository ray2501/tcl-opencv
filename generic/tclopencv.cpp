...
void OPENCV_Thread_Exit(ClientData clientdata)
{
    if(cv_hashtblPtr) {
        MatrixInfo *matrix_info;
        CvCallbackInfo *callbackinfo;
        VideoCaptureInfo *vc_info;
        VideoWriterInfo *vw_info;
        CascadeClassifierInfo *cas_info;
        QRCodeDetectorInfo *qr_info;
        TermCriteriaInfo *term_info;
        Tcl_HashEntry *hashEntryPtr;
        Tcl_HashSearch search;
        char *key;

        for (hashEntryPtr = Tcl_FirstHashEntry (cv_hashtblPtr, &search);
             hashEntryPtr != NULL;
             hashEntryPtr = Tcl_NextHashEntry(&search)) {
             key = (char *) Tcl_GetHashKey (cv_hashtblPtr, hashEntryPtr);

             if (strncmp(key, "cv-callback", 11)==0) {
                 callbackinfo = (CvCallbackInfo *) Tcl_GetHashValue(hashEntryPtr);

                 ckfree(callbackinfo);
             } else if (strncmp(key, "cv-mat", 6)==0) {
                 matrix_info = (MatrixInfo *) Tcl_GetHashValue(hashEntryPtr);

                 delete matrix_info->matrix;
                 ckfree(matrix_info);
             } else if (strncmp(key, "cv-videoc", 9)==0) {
                 vc_info = (VideoCaptureInfo *) Tcl_GetHashValue(hashEntryPtr);

                 vc_info->capture->release();
                 delete vc_info->capture;
                 ckfree(vc_info);
             } else if (strncmp(key, "cv-videow", 9)==0) {
                 vw_info = (VideoWriterInfo *) Tcl_GetHashValue(hashEntryPtr);

                 vw_info->writer->release();
                 delete vw_info->writer;
                 ckfree(vw_info);
             } else if (strncmp(key, "cv-odetect", 10)==0) {
                 cas_info = (CascadeClassifierInfo *) Tcl_GetHashValue(hashEntryPtr);

                 delete cas_info->classifier;
                 ckfree(cas_info);
             } else if (strncmp(key, "cv-qdetect", 10)==0) {
                 qr_info = (QRCodeDetectorInfo *) Tcl_GetHashValue(hashEntryPtr);

                 delete qr_info->qrcodedetector;
                 ckfree(qr_info);
             } else if (strncmp(key, "cv-term", 7)==0) {
                 term_info = (TermCriteriaInfo *) Tcl_GetHashValue(hashEntryPtr);

                 delete term_info->termCriteria;
                 ckfree(term_info);
             }
        ...
        Tcl_DeleteHashTable(cv_hashtblPtr);
        ckfree(cv_hashtblPtr);
    }
}
...
Opencv_Init(Tcl_Interp *interp)
{
    ...
    cv_hashtblPtr = (Tcl_HashTable *) ckalloc(sizeof(Tcl_HashTable));
    Tcl_InitHashTable(cv_hashtblPtr, TCL_STRING_KEYS);
    matrix_count = 0;
    video_count = 0;
    callback_count = 0;
    detect_count = 0;
    ...
    Tcl_CreateThreadExitHandler(OPENCV_Thread_Exit, (ClientData)NULL);
